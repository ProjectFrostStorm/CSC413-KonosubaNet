...
For training, we used teacher forcing. So the input is **sequence[:-1]** and the target is **sequence[1:]**. For loss function and optimizer, we use cross-entrophy, and ADAM respectivly. 
## Training Curve
...
When analyzed from a qualitative point of view, our use of the bleu score is not a perfect choice. Usually, bleu score is comparing output from a model and some reference texts. In our case, we don’t have a reference text. We trained our model using the 17 volumes of the novel, comparing our model with the 18th volume is not a good way to conclude whether our model performed well or not. Our bleu score function is comparing every 3*(random # of sentences in output of sample_sequence) sentences of the novel (starting from volume1) to every n=(random # of sentences in output of sample_sequence) sentences of the generated text. We are not complaring entire generated text to all 17 volume beause it would take too long to iterate. The score we get at epoch 300 is 0.082595. 

From the following graph, we see that the bleu scores are iterating mainly between 0.07 and 0.12, with someiterations going to 0 and some going to 0.15. Bleu scores are not positively correlated with epochs, it does not always increase as epoch increases. This can be explained by the definition of bleu score, which usually compares the generated text with a single truth. Since we are only comparing how close is our output from the training data, then the shape of this graph can be explained.
...
In order to increase model performance, we could train using computers with more memory, but also set *max_seq_length* to 512. This means that instead of generating from any length, our modele only remembers the last 512 characters and loses context of everything before. This of course creates a problem for capturing long term dependencies, but it would help with the memory problem.
...
# Authors
Sally Zhu (ethical concerns, interpret results, exaplin training hyperparamters, justification of result)
